...
- **常规**：服务端消息语言和管理员权限等级。
- **显示**：默认隐藏、默认轮转与轮转间隔。
- **榜单**：包含默认展开的“内置榜单”和“自定义脚本榜单”两组；每榜都有“已加载/已停用”“默认显示/默认隐藏”和“重置”三个控件。自定义组还提供“刷新”按钮，只会原地更新脚本预览，不会应用到正在运行的服务器。
...
界面布局会递归解析 `StarryListConfigData` record 自动生成。受支持的普通字段默认使用 `starrylist.config.<完整字段路径>` 作为标签和提示键，现有字段则通过路径 metadata 保持原有文字。新增普通类型时只需注册 `StarryListConfigEntryBuilder`；字段路径可覆盖通用类型 builder，需要联合处理多个字段的顶层 record 则可注册 `StarryListConfigSectionBuilder`。“全部”和独立分类始终创建不同 widget 实例，并统一绑定到 `StarryListConfigEditorModel`，从而在同步值的同时隔离焦点、输入法组合、动态行、校验与重置状态。

ModMenu 源码按职责归类：`screen` 管理界面生命周期与布局，`builder` 管理扩展接口和分派，`model` 管理 record 映射与编辑状态，`section` 管理跨字段区段，`entry` 再细分为 `common`、`scalar`、`board` 和 `blacklist` 控件。
